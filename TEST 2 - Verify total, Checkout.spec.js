...
test('Verify total', async ({ page }) => {
  ...
  const result = page.locator('data-test="total-label"');
  await expect('data-test="total-label"').toMatch('data-test="total-label"');
  
  const value = 'text=Total: $95.01';
  
  await expect(value).toContain("text=Total: $95.01");  // class="summary_total_label"  .summary_total_label  data-test="total-label"
  
  await page.locator('[data-test="finish"]').click();
  
  // Expect the message to be "Thank you for your order!":
